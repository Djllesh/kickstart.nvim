...
return {
  ...
  config = function()
    ...
    -- Define colors (use your preferred hex codes or terminal colors)
    -- Example colors:
    ...
    local function update_adjacent_line_nr_highlights()
      ...
      for dist, hl_group in pairs(hl_groups) do
        local target_line_1based = line + dist
    ...
    vim.api.nvim_create_autocmd({ 'CursorMoved', 'CursorMovedI', 'BufWinEnter', 'WinEnter' }, {
      ...
      callback = function()
        ...
        if vim.bo.buftype == '' and vim.api.nvim_win_is_valid(vim.api.nvim_get_current_win()) then
          pcall(update_adjacent_line_nr_highlights) -- Use pcall for safety
        end
      end,
